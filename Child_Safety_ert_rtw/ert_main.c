...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Child_Safety_M) == (NULL));
  while (runModel) {
    MW_sem_wait(&baserateTaskSem);
    Child_Safety_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Child_Safety_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Child_Safety_M, "stopping the model");
  runModel = 0;
}
...
{
  int argc = 4;
  char *argv[4] = { "Child_Safety", "-w", "-port", "17725" };

  my_android_init(pEnv, pObj, pArgv, pMainActivity);
  rtmSetErrorStatus(Child_Safety_M, 0);

  /* Initialize model */
  Child_Safety_initialize();
